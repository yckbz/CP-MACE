...
class Configuration:
    ...
    potential: Optional[float] = None  # eV
    electron: Optional[float] = None  # eV
    ...
    potential_weight: float = 1.0  # weight of config virial in loss
    electron_weight: float = 1.0  # weight of config virial in loss
...
def config_from_atoms(
    ...
    electron_key = 'electron'
    potential = atoms.info.get(potential_key, None)
    electron = atoms.info.get(electron_key, None)
    ...
    potential_weight = atoms.info.get("config_potential_weight", 1.0)
    electron_weight = atoms.info.get("config_electron_weight", 1.0)

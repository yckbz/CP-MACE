...
def valid_err_log(
    ...
    # uncertainty
    mean_e_unc = eval_metrics.get("mean_e_unc", None)
    mean_p_unc = eval_metrics.get("mean_p_unc", None)

    if mean_e_unc is not None:
        e_unc_str = f", energy_unc={mean_e_unc:8.4f}"
    else:
        e_unc_str = ""

    if mean_p_unc is not None:
        p_unc_str = f", potential_unc={mean_p_unc:8.4f}"
    else:
        p_unc_str = ""
    ...
    elif log_errors == "Fermi_PerAtomRMSE":
        error_e = eval_metrics["rmse_e_per_atom"] * 1e3
        error_f = eval_metrics["rmse_f"] * 1e3
        error_p = eval_metrics["rmse_p"]
        logging.info(
            f"{inintial_phrase}: head: {valid_loader_name}, loss={valid_loss:8.8f}, RMSE_E_per_atom={error_e:8.3f} meV, RMSE_F={error_f:8.3f} meV / A, RMSE_P={error_p:8.4f} V, {e_unc_str}{p_unc_str}"
        )
    elif (
...
def take_step(
    ...
    output = model(
        ...
        compute_potential=output_args["potential"],
...
class MACELoss(Metric):
    def __init__(self, loss_fn: torch.nn.Module):
        ...
        self.add_state("delta_ps", default=[], dist_reduce_fx="cat")
        ...
        # uncertainty
        self.add_state("energy_unc", default=[], dist_reduce_fx="cat")  # energy uncertainty
        self.add_state("potential_unc", default=[], dist_reduce_fx="cat") # potential uncertainty
    ...
    def update(self, batch, output):  # pylint: disable=arguments-differ
        ...
        if output.get("potential") is not None and batch.potential is not None:
            if batch.potential_flag is not None:
                # Convert potential_flag to a boolean mask.
                mask = batch.potential_flag.bool()
                # If all entries are zero, return 0.
                if mask.sum() == 0:
                    pass
                else:
                    self.delta_ps.append(batch.potential[mask] - output["potential"][mask])
        ...
        if "energy_logvar" in output:
            e_sigma = torch.exp(0.5 * output["energy_logvar"])
            self.energy_unc.append(e_sigma)
        if "potential_logvar" in output:
            p_sigma = torch.exp(0.5 * output["potential_logvar"])
            self.potential_unc.append(p_sigma)
    ...
    def compute(self):
        ...
        if self.E_computed:
            ...
            if len(self.delta_ps) != 0:
                delta_ps = self.convert(self.delta_ps)
                aux["rmse_p"] = compute_rmse(delta_ps)
        ...
        if len(self.energy_unc) > 0:
            e_unc = self.convert(self.energy_unc)  # 把 list of Tensors -> 1D numpy array
            aux["mean_e_unc"] = np.mean(e_unc)
            aux["std_e_unc"] = np.std(e_unc)
            aux["max_e_unc"] = np.max(e_unc)
            aux["min_e_unc"] = np.min(e_unc)
            aux["q95_e_unc"] = compute_q95(e_unc)

        if len(self.potential_unc) > 0:
            p_unc = self.convert(self.potential_unc) 
            aux["mean_p_unc"] = np.mean(p_unc)
            aux["std_p_unc"] = np.std(p_unc)
            aux["max_p_unc"] = np.max(p_unc)
            aux["min_p_unc"] = np.min(p_unc)
            aux["q95_p_unc"] = compute_q95(p_unc)

...
def run(args: argparse.Namespace) -> None:
    ...
    #     rank = int(0)
    if args.distributed:
        # First, try the standard torchrun environment variables
        if {"RANK", "LOCAL_RANK", "WORLD_SIZE", "MASTER_ADDR", "MASTER_PORT"}.issubset(os.environ):
            world_size = int(os.environ["WORLD_SIZE"])
            local_rank = int(os.environ["LOCAL_RANK"])
            rank = int(os.environ["RANK"])
        else:
            # Fall back to the SLURM‑based helper class (original behaviour)
            try:
                distr_env = DistributedEnvironment()
                world_size = distr_env.world_size
                local_rank = distr_env.local_rank
                ...
                logging.error(f"Failed to initialize distributed environment: {e}")
                return
        # Ensure essential variables exist when launched without SLURM
    ...
    if args.model == "AtomicDipolesMACE":
        ...
        if args.model == "EnergyDipolesMACE":
            ...
            args.compute_virials = False
            args.compute_stress = False
        elif args.model == "FermiMACE":
            args.compute_energy = True
            args.compute_potential = True
    ...
    electron_values = []
    potential_values = []
    if args.compute_potential == True:
        for config in head_config.collections.train:
            if config.potential != args.empty_potential_value:
                electron_values.append(config.electron)
                potential_values.append(config.potential)
        args.mean_electron = float(np.mean(electron_values))
        args.mean_potential = float(np.mean(potential_values))
        ...
        if rank == 0:
            ...
            if swa_eval:
                ...
                try:
                    ...
                    print("TorchScript compiling failed：", e)
                    raise
#                    pass
